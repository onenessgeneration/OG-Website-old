...
import { fetchVisibleTestimonials, type Testimonial } from "@/lib/people";

const Slider = ((SliderModule as unknown) as { default?: typeof SliderModule }).default ?? SliderModule;

// Static fallback used only while the DB is empty — first-time setup UX so the
// section doesn't look broken before an admin adds any real testimonials.
const fallback: Testimonial[] = [
  {
    ...
    tag: "Testimonial",
    actual_testimonial:
      "It was when I truly saw myself as who I truly am, as a whole, not just what I pretend to be, in the external, did I become a better individual. Oneness helped me actually see myself.",
    quote:
  ...
  },
  {
    ...
    tag: "Testimonial",
    actual_testimonial:
      "A space for healing, transformation, and inner connection. I've learned to appreciate even the smallest things in life, and that has completely shifted my attitude toward life.",
    quote: "Oneness Generation brought healing and a new appreciation for life's simple joys.",
    image_url:
...
export default function NewTestimonials() {
  ...
  const display = rows === null ? fallback : rows.length === 0 ? fallback : rows;

  const settings = {
    dots: false,
    infinite: display.length > 2,
    speed: 1000,
    slidesToShow: Math.min(2, display.length),
    slidesToScroll: 1,
  ...
  const avatarFor = (t: Testimonial) =>
    t.image_url ||
    `https://api.dicebear.com/9.x/initials/svg?seed=${encodeURIComponent(t.participant)}&backgroundColor=b78036&textColor=ffffff`;
        ...
        <ClientOnly fallback={<div className="md:w-2/3 min-h-[200px]" />}>
          ...
          >
            {display.map((d, index) => {
              ...
              return (
                <div key={d.id} className="md:px-10 px-2">
                  <div className="relative bg-tan shadow-md rounded-lg flex flex-col md:p-6 p-3 md:space-y-4 space-y-2 h-full">
                    ...
                    <div className="lg:pl-12">
                      <p className="text-[11px] text-gray-500">{truncate(d.tag ?? "Testimonial", 20)}</p>
                    </div>

                    <p className="text-gray-500 md:text-[12px] text-[11px] md:pl-12">
                      {isExpanded ? d.actual_testimonial : truncate(d.actual_testimonial, 250)}
                      {d.actual_testimonial.length > 250 && (
                        <button
                          onClick={() =>
                    ...
                    {d.quote && (
                      <p className="text-gray-500 text-[12px] md:pl-12">
                        {isQuoteExpanded ? d.quote : truncate(d.quote, 200)}
                        {d.quote.length > 200 && (
                          <button
                            onClick={() =>
